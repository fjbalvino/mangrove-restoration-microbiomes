#!/usr/bin/env Rscript
# ============================================================
# 06_03_environment_mixed_models.R
# Status: REVIEW CANDIDATE; see docs/REPRODUCIBILITY.md
# Five separately fitted axes; BH pooled across axes within common, interaction and depth-slope families.
# Inputs (source expressions; complete list in docs/contracts/06_03_environment_mixed_models.json):
...
#   writeLines(capture.output(sessionInfo()), file.path(run, "logs", "603c_sessionInfo.txt"))
#   write_tsv(data.frame(input = paths, md5 = unname(hashes)), file.path(run, "tables", "603c_inputs.tsv"))
#   write_tsv(m[, setdiff(names(m), "axis_z")], file.path(run, "tables", "603c_metadata_used.tsv"))
#   write_tsv(scaling, file.path(run, "tables", "603c_axis_audit.tsv"))
#   write_tsv(data.frame(axis = rownames(correlations), correlations, check.names = FALSE),
#   writeLines(c(
#   saveRDS(part, file.path(run, "checkpoints", sprintf("axis%02d_batch%04d.rds", a, h)))
#   write_tsv(r, file.path(run, "tables", "603c_all_results.tsv.gz"))
#   write_tsv(diagnostics, file.path(run, "tables", "603c_fit_diagnostics.tsv"))
# Algorithmic provenance:
# Separate environmental-axis REML mixed models; predictor-depth interactions; declared pooled BH families.
...
# Source SHA-256: 8800c1c578c6a5b0f301c88f38a97d4e6a5d7246cbb895f29a8833203f59f0a5
...
# Figure 4: five environmental blocks; 51 samples, 17 complete profiles.
# Does not average depths, refit MHI, re-centre CLR, or run enrichment.
options(stringsAsFactors = FALSE, warn = 1)
NAME <- "603c_asociaciones_bloques_por_profundidad"
...
required <- c("sample_id", "profile_id", "locality", "restoration4", "depth_cm", "MHI_local",
  "vegetation_landscape_PC1", "water_inundation_PC1", "moisture_stress_PC1",
...
axes <- c("vegetation_landscape_PC1", "water_inundation_PC1", "moisture_stress_PC1",
          "physicochemical_PC1", "nutrients_redox_PC1")
forms <- list(common = y ~ axis_z + depth + locality + (1 | profile_id),
              interaction = y ~ axis_z * depth + locality + (1 | profile_id))
scaling <- vector("list", length(axes))
for (j in seq_along(axes)) {
  x <- as.numeric(m[[axes[j]]])
  need(all(is.finite(x)) && sd(x) > 0, paste("Eje no valido:", axes[j]))
  m[[axes[j]]] <- x
  m$axis_z <- (x - mean(x)) / sd(x)
  for (f in forms) {
    X <- model.matrix(lme4::nobars(f), transform(m, y = 0))
    need(qr(X)$rank == ncol(X), paste("Rango deficiente:", axes[j]))
  }
  within_ss <- sum(vapply(profiles, function(ii) sum((x[ii] - mean(x[ii]))^2), numeric(1)))
  scaling[[j]] <- data.frame(axis = axes[j], mean = mean(x), sd = sd(x), n = length(x),
    profiles_with_variation = sum(vapply(profiles, function(ii) diff(range(x[ii])) > 1e-10, logical(1))),
    within_profile_SS_fraction = within_ss / sum((x - mean(x))^2),
    common_rank = qr(model.matrix(~ axis_z + depth + locality, m))$rank,
    interaction_rank = qr(model.matrix(~ axis_z * depth + locality, m))$rank)
}
scaling <- data.table::rbindlist(scaling)
selection <- read_tsv(paths[4])
...
design <- function(depth_value, x) {
  ...
  d$axis_z <- x
  model.matrix(~ axis_z * depth + locality, d)
}
...
analyse <- function(i) {
  ...
  k <- ncol(model.matrix(~ axis_z + depth + locality, m))
  L <- numeric(k)
  L[match("axis_z", colnames(model.matrix(~ axis_z + depth + locality, m)))] <- 1
  ...
  rows <- list(contrast_row(common, L, "axis_common"),
               contrast_row(interaction, interaction_L, "axis_by_depth", joint = TRUE))
  for (j in 1:3) rows[[length(rows) + 1L]] <-
    contrast_row(interaction, slopes[j, ], "axis_depth_slopes", c(5L, 20L, 40L)[j])
...
# One synthetic contrast check of the unchanged engine before fitting any proteins.
set.seed(603)
u <- rnorm(17, sd = 3)
test_y <- 1.2 * m$axis_z + 0.8 * m$axis_z * (m$depth_cm == 20) -
  0.5 * m$axis_z * (m$depth_cm == 40) + u[as.integer(m$profile_id)] + rnorm(51, sd = 0.5)
tf <- fit_one(test_y, "interaction")
need(!is.null(tf$fit) && tf$status == "ok", "Fallo en prueba sintetica del modelo")
et <- as.data.frame(emmeans::emtrends(tf$fit, ~ depth, var = "axis_z",
...
need(max(abs(drop(slopes %*% b) - et$axis_z.trend)) < 1e-7 &&
...
for (d in c("tables", "checkpoints", "logs")) dir.create(file.path(run, d), recursive = TRUE)
writeLines(capture.output(sessionInfo()), file.path(run, "logs", "603c_sessionInfo.txt"))
...
write_tsv(m[, setdiff(names(m), "axis_z")], file.path(run, "tables", "603c_metadata_used.tsv"))
write_tsv(scaling, file.path(run, "tables", "603c_axis_audit.tsv"))
correlations <- cor(m[, c(axes, "MHI_local")])
write_tsv(data.frame(axis = rownames(correlations), correlations, check.names = FALSE),
          file.path(run, "tables", "603c_predictor_correlations.tsv"))
writeLines(c(
  "Five environmental axes fixed before fitting; no refitting or sign changes.",
  "Response: full-catalog CLR, pseudocount 1; same variance-selected 100000 proteins.",
  "Separate models per axis; no HI, HFR, MHI, restoration stage or other axes as covariates.",
  "Common: CLR ~ axis_z + depth_factor + locality + (1 | profile_id).",
  "Secondary: CLR ~ axis_z * depth_factor + locality + (1 | profile_id).",
  "BH jointly over all five axes: 500000 common, 500000 interactions, 1500000 slopes.",
  "Primary: status ok only; non-usable P=1 internally, q=NA in output.",
  "Diagnostic: include singular without other warnings; same complete families.",
  "Diagnostic q values do not replace primary discoveries or validate recovery.",
  "Pilot: 500 evenly spaced variance ranks per axis, no FDR or hit selection."
), file.path(run, "logs", "603c_protocol.txt"))
cat("Output:", run, "\nMode:", mode, "| Features per axis:", length(idx), "| Workers:", workers, "\n")
started <- proc.time()[3]
chunks <- split(idx, ceiling(seq_along(idx) / 100L))
axis_results <- vector("list", length(axes))
for (a in seq_along(axes)) {
  axis_name <- axes[a]
  m$axis_z <- (m[[axis_name]] - scaling$mean[a]) / scaling$sd[a]
  cat("\nEje", a, "/", length(axes), ":", axis_name, "\n")
  ...
  for (h in seq_along(chunks)) {
    ...
    saveRDS(part, file.path(run, "checkpoints", sprintf("axis%02d_batch%04d.rds", a, h)))
    all_results[[h]] <- part
    done_axis <- min(h * 100L, length(idx))
    done_total <- (a - 1L) * length(idx) + done_axis
    elapsed <- proc.time()[3] - started
    cat(format(Sys.time(), "%H:%M:%S"), "|", axis_name, "|", done_axis, "/", length(idx),
        "| min:", round(elapsed / 60, 1), "| estimacion full 5 ejes:",
        round(elapsed / done_total * 500000 / 3600, 2), "h\n")
    flush.console()
  }
  axis_results[[a]] <- data.table::rbindlist(all_results)
  rm(all_results, out, part)
}
r <- data.table::rbindlist(axis_results)
rm(axis_results)
need(nrow(r) == 5L * length(idx) * length(axes), "Numero de resultados inesperado")
need(!anyDuplicated(r[, c("axis", "feature_id", "family", "depth_cm")]), "Resultados duplicados")
r$usable <- r$status == "ok" & is.finite(r$p)
r$diagnostic_usable <- r$status %in% c("ok", "singular") & is.finite(r$p)
r$q <- NA_real_
r$q_including_singular_diagnostic <- NA_real_
if (mode == "full") {
  for (fam in unique(r$family)) {
    ...
    pp_diagnostic <- ifelse(r$diagnostic_usable[jj], r$p[jj], 1)
    qq_diagnostic <- p.adjust(pp_diagnostic, method = "BH")
    r$q_including_singular_diagnostic[jj] <- ifelse(r$diagnostic_usable[jj], qq_diagnostic, NA_real_)
  }
}
r$direction <- ifelse(is.na(r$estimate), NA_character_,
                      ifelse(r$estimate > 0, "higher_axis_values", "lower_axis_values"))
r$passes_q_0_10 <- !is.na(r$q) & r$q <= 0.10
diagnostics <- r[, .(n_tests = .N), by = .(axis, family, depth_cm, status, singular)]
families <- r[, .(n_tests = .N, n_usable = sum(usable),
  n_q_0_10 = if (mode == "full") sum(passes_q_0_10) else NA_integer_,
  n_q_diagnostic_0_10 = if (mode == "full")
    sum(q_including_singular_diagnostic <= 0.10, na.rm = TRUE) else NA_integer_), by = family]
by_axis <- r[, .(n_tests = .N, n_usable = sum(usable),
  n_q_0_10 = if (mode == "full") sum(passes_q_0_10) else NA_integer_), by = .(axis, family)]
write_tsv(r, file.path(run, "tables", "603c_all_results.tsv.gz"))
write_tsv(diagnostics, file.path(run, "tables", "603c_fit_diagnostics.tsv"))
write_tsv(families, file.path(run, "tables", "603c_test_families.tsv"))
write_tsv(by_axis, file.path(run, "tables", "603c_summary_by_axis.tsv"))
print(by_axis)
need(identical(unname(hashes), unname(tools::md5sum(paths))), "Entradas cambiaron durante ejecucion")
need(all(by_axis$n_usable > 0), "Ninguna prueba utilizable en al menos un eje/familia; revisar")
status <- if (mode == "pilot") "PILOT_COMPLETE" else "PASS"
write_tsv(data.frame(status = status, mode = mode, samples = 51, profiles = 17,
  axes = length(axes), features_per_axis = length(idx), catalog_features = 1475487,
  pseudocount = 1, depth_averaged = FALSE, axes_refitted = FALSE,
  selection_uses_predictors = FALSE, HI_HFR_MHI_in_models = FALSE, fit_method = "REML",
  test_method = "Satterthwaite", random_effect = "profile_intercept", FDR_computed = mode == "full",
  FDR_scope = "joint_five_axes_within_each_test_family", q_cutoff = 0.10,
  non_ok_tests = sum(!r$usable), elapsed_minutes = (proc.time()[3] - started) / 60),
  file.path(run, "tables", "603c_run_summary.tsv"))
